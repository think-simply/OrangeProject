...
Then("Work shift {string} has been created successfully", async function (text) {
...
When("User removes all work-shifts contain text {string}", async function (text) {
    await workShiftsPage.removeMultiShift(text);
})
Then("All work-shifts contain text {string} have been deleted", async function (text) {
    await workShiftsPage.verifyRemoveMultiShift(text);
});
When("User create a new work shift with name {string} successfully", async function (shift) {
    await workShiftsPage.accessWorkShift();
    await workShiftsPage.clickAddBtn();
    await workShiftsPage.addDataWorkShift(shift);
    await workShiftsPage.clickSave();
