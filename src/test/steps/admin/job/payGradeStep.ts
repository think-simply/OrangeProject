...
When(
  "User create Pay Grade with Pay Grade Name: {string}",
  async function (payGradeName) {
    await payGradePage.userCreateNewPayGradePage(payGradeName);
  }
);
Then(
  "New Pay Grade has been created successfully with name: {string}",
  async function (payGradeName) {
    await payGradePage.verifyCreatePayGradeSuccessfully(payGradeName);
  }
);
When(
  "User update Pay Grade from Pay Grade Name: {string} to {string}",
  async function (payGradeName, newPayGrade) {
    await payGradePage.editPayGrade(payGradeName, newPayGrade);
  }
);
Then(
  "Pay Grade has been updated successfully to {string}",
  async function (newName) {
    await payGradePage.verifyUpdatePayGradeSuccessfully(newName);
  }
);
When(
  "User delete Pay Grade from Pay Grade Name: {string}",
  async function (payGradeName) {
    await payGradePage.deletePayGrade(payGradeName);
  }
);
Then(
  "Pay Grade has been deleted successfully with Pay Grade Name: {string}",
  async function (payGradeName) {
