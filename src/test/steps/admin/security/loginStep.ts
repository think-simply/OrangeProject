...
Given('I am on the login page', async function () {
    await pageFixture.page.goto(process.env.LOGIN_URL);
});
...
Then('I should see an error message', async function () {
    const errorMessage = await pageFixture.page.locator('.error-message');
...
When("User try to access admin menu as guest", { timeout: 20000 }, async () => {
    pageFixture.page.goto('https://buianthai.online/orangehrm/web/index.php/admin/viewSystemUsers');
});
Then("User cannot reach to group and role page", { timeout: 20000 }, async () => {
    await expect(pageFixture.page.locator('//span[text()="This page isn’t working"]')).toBeVisible({ timeout: 5000 });
});